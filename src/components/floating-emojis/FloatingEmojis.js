...
const FloatingEmojis = ({
  ...
  const onNewEmoji = useCallback(
    (x, y) => {
      ...
      timeout.current = setTimeout(clearEmojis, duration * 1.5);

      const prevEmojis = emojis.slice(emojis.length - 30);
      const newEmoji = {
        // if a user has mashed the button 7 times, they deserve a 🌈️ rainbow
        emojiToRender: touched && emojis.length % 7 === 0 ? 'rainbow' : emoji,
        x: x ? x - getRandomNumber(-20, 20) : `${getRandomNumber(...range)}%`,
        y: y || 0,
      };

      setEmojis([...prevEmojis, newEmoji]);
